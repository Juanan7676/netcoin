...
inputAddress = config:read()
outputAddress = config:read()
modem1 = component.proxy(component.get(inputAddress))
modem2 = component.proxy(component.get(outputAddress))
event = require("event")
...
while true do
  local _,lm,recv,port,_,msg = event.pull("modem") -- Router responds: <port> on success, nil on fail
  ...
  if port ~= 9999 and index~= false then
    if connections[index][5]=="false" then
        ...
        modem2.send(connections[index][4],9999,"DELIVER,"..connections[index][3]..","..myIP..","..msg)
    else 
      print("Sending FORWARD to address " .. connections[index][4])
      modem2.send(connections[index][4],9999,"FORWARD,".. connections[index][2]..",".. connections[index][3]..","..myIP..","..msg)
    end
  else
      ...
      if result ~= false then modem1.send(recv,9999,result[1]) end
    elseif arr[1]=="DELIVER" then
      modem1.broadcast(tonumber(arr[2]),arr[3].."!"..arr[4])
      ...
      for _,mapping in pairs(mappings) do
        if ip[1] >= mapping[1][1] and ip[1] <= mapping[1][2] and ip[2] >= mapping[2][1] and ip[2] <= mapping[2][2] and
           ip[3] >= mapping[3][1] and ip[3] <= mapping[3][2] and ip[4] >= mapping[4][1] and ip[4] <= mapping[4][2] then
              if mapping[6]==false and lm==inputAddress then modem2.send(mapping[5],9999,"DELIVER,"..arr[3]..","..arr[4]..","..arr[5])
              elseif mapping[6]==false then modem1.send(mapping[5],9999,"DELIVER,"..arr[3]..","..arr[4]..","..arr[5])
              elseif lm==inputAddress then modem2.send(mapping[5],9999,"FORWARD,"..arr[2]..","..arr[3]..","..arr[4]..","..arr[5])
              else modem1.send(mapping[5],9999,"FORWARD,"..arr[2]..","..arr[3]..","..arr[4]..","..arr[5]) end
        end
      end
    end
  end
