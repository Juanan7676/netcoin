...
function BlockFactory:createChain(length, targets)
    local blocks = {
        BlockFactory:new():setHeight(0):setTarget(targets[1]):calculateUUID():calculateValidNonce():create()
    }

    for k = 2, length do
        table.insert(
            blocks,
            BlockFactory:new():setHeight(k - 1):setPreviousBlock(blocks[k - 1]):setTarget(targets[k]):calculateUUID():calculateValidNonce(
