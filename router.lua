...
while true do
  local _,_,recv,port,_,msg = event.pull("modem") -- Router responds: <port> on success, nil on fail
  ...
  if port ~= 9999 and index~= false then
    if connections[index][5]=="false" then
      ...
      modem2.send(connections[index][5],9999,"FORWARD,".. connections[index][2]..",".. connections[index][3]..","..myIP..","..msg)
    end
  else
      ...
      for _,mapping in pairs(mappings) do
        if ip[1] >= mapping[1][1] and ip[1] <= mapping[1][2] and ip[2] >= mapping[2][1] and ip[2] <= mapping[2][2] and
           ip[3] >= mapping[3][1] and ip[3] <= mapping[3][2] and ip[4] >= mapping[4][1] and ip[4] <= mapping[4][2] then
              if mapping[6]==false then modem2.send(mapping[5],9999,"DELIVER,"..arr[3]..","..arr[4]..","..arr[5])
            else modem2.send(mapping[5],9999,"FORWARD,"..arr[2]..","..arr[3]..","..arr[4]..","..arr[5]) end
        end
      end
    end
  end
